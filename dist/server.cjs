...
server.tool(
  ...
  {
    name: import_zod.z.string().describe("Name of the color variable. Use '/' for grouping, e.g., 'primary/50'"),
    r: import_zod.z.number().min(0).max(1).describe("Red component (0-1)"),
    g: import_zod.z.number().min(0).max(1).describe("Green component (0-1)"),
    b: import_zod.z.number().min(0).max(1).describe("Blue component (0-1)"),
    a: import_zod.z.number().min(0).max(1).optional().describe("Alpha/opacity (0-1, default: 1)"),
    collectionName: import_zod.z.string().optional().describe("The name of the variable collection to use or create. Default: 'TalkToFigmaMCP Variables'")
  },
...
server.tool(
  ...
  "Create or update multiple Figma color variables at once. Group using '/' in names.",
  {
    variables: import_zod.z.array(import_zod.z.object({
      name: import_zod.z.string().describe("Name of the color variable, e.g., 'primary/50'"),
      r: import_zod.z.number().min(0).max(1).describe("Red component (0-1)"),
      g: import_zod.z.number().min(0).max(1).describe("Green component (0-1)"),
      b: import_zod.z.number().min(0).max(1).describe("Blue component (0-1)"),
      a: import_zod.z.number().min(0).max(1).optional().describe("Alpha/opacity (0-1, default: 1)")
...
server.tool(
  ...
  {
    name: import_zod.z.string().describe("Name of the color style. Use '/' for grouping, e.g., 'teal/teal-50'"),
    r: import_zod.z.number().min(0).max(1).describe("Red component (0-1)"),
    g: import_zod.z.number().min(0).max(1).describe("Green component (0-1)"),
    b: import_zod.z.number().min(0).max(1).describe("Blue component (0-1)"),
    a: import_zod.z.number().min(0).max(1).optional().describe("Alpha/opacity (0-1, default: 1)")
...
server.tool(
  ...
  {
    styles: import_zod.z.array(import_zod.z.object({
      name: import_zod.z.string().describe("Name of the color style, e.g., 'teal/teal-50'"),
      r: import_zod.z.number().min(0).max(1).describe("Red component (0-1)"),
      g: import_zod.z.number().min(0).max(1).describe("Green component (0-1)"),
      b: import_zod.z.number().min(0).max(1).describe("Blue component (0-1)"),
      a: import_zod.z.number().min(0).max(1).optional().describe("Alpha/opacity (0-1, default: 1)")
  ...
  async ({ styles }) => {
    try {
      const mappedStyles = styles.map((s) => ({
        name: s.name,
        color: { r: s.r, g: s.g, b: s.b, a: s.a !== void 0 ? s.a : 1 }
      }));
...
server.tool(
  ...
  "Apply an existing color style (paint style) to a node's fill in Figma. Use get_styles to find style IDs first.",
  {
    ...
    styleId: import_zod.z.string().describe("The ID of the paint style to apply (from get_styles)")
  },
...
server.tool(
  ...
  {
    styleId: import_zod.z.string().describe("The ID of the style to rename."),
    newName: import_zod.z.string().describe("The new name for the style.")
  },
...
server.tool(
  ...
  {
    fromStyleId: import_zod.z.string().describe("The ID of the style to replace."),
    toStyleId: import_zod.z.string().describe("The ID of the new style to use.")
  },
  async ({ fromStyleId, toStyleId }) => {
    try {
...
server.tool(
  ...
  {
    channel: import_zod.z.string().describe("The name of the channel to join").default("figma-default")
  },
  async ({ channel }) => {
    try {
      if (!channel) {
...
function startEmbeddedSocketServer(port = 3055) {
  ...
  embeddedWsServer = new import_ws.WebSocketServer({ server: httpServer });
      ...
      try {
        ...
        if (data.type === "join") {
          ...
          channelClients.forEach((client) => {
            if (client !== clientWs && client.readyState === import_ws.default.OPEN) {
